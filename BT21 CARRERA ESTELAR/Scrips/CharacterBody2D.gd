...
func aplicar_super_velocidad(multiplicador: float, duracion: float):
	var velocidad_original = velocidad # Guardamos 300.0
	velocidad = velocidad * multiplicador # Aumentamos (ej. 600.0)
	
	# Efecto visual: Se pone un poco amarillo/dorado para saber que tiene el poder
	modulate = Color(1.5, 1.5, 0.5) 
	
	print("¡Súper velocidad activada!")
	
	# Esperamos los 30 segundos sin detener el resto del juego
